...
class Subscribers extends Controller
{
    ...
    public $requiredPermissions = ['indikator.newsletter.subscribers'];
    ...
    public function __construct()
    {
        ...
        BackendMenu::setContext('Indikator.Newsletter', 'newsletter', 'subscribers');
    }
    ...
    public function confirm($id = null, $hash = null)
    {
        ...
        if ($subscriber == null) {
            Flash::failed(Lang::get('indikator.newsletter::lang.flash.subscriber_confirmation_token_invalid'));
            return Redirect::to('/');
        }

        if ($subscriber->status == 3 && $subscriber->confirmation_hash == $hash) {
            if ($subscriber->registered_at < Date::now()->subDay()) {
                Flash::error(Lang::get('indikator.newsletter::lang.flash.subscriber_confirmation_token_expired'));
                return Redirect::to('/');
            }
            ...
            Flash::success(Lang::get('indikator.newsletter::lang.flash.subscriber_confirmation'));
        }
        else if ($subscriber->status == 1) {
            Flash::success(Lang::get('indikator.newsletter::lang.flash.subscriber_already_confirmed'));
        }
        else {
            Flash::error(Lang::get('indikator.newsletter::lang.flash.subscriber_confirmation_token_invalid'));
        }
    ...
    public function onShowStat()
    {
        ...
        $this->vars['confirmed_at']    = ($subscriber->confirmed_at) ? $subscriber->confirmed_at : '<em>'.e(trans('indikator.newsletter::lang.form.no_data')).'</em>';
        $this->vars['unsubscribed_at'] = ($subscriber->unsubscribed_at) ? $subscriber->unsubscribed_at : '<em>'.e(trans('indikator.newsletter::lang.form.no_data')).'</em>';
